...
def Generate_moment_matrix(monomial_list, HermitianVars, ComplexVars):
    ...
    B0, B1, B2, Z0, Z1, Z2, Z3, Z0T, Z1T, Z2T, Z3T = sp.symbols("B0 B1 B2 Z0 Z1 Z2 Z3 Z0T Z1T Z2T Z3T", commutative=False)
    local_dict = {
        'B0': B0, 'B1': B1, 'B2': B2,
        'Z0': Z0, 'Z1': Z1, 'Z2': Z2, 'Z3': Z3,
        'Z0T': Z0T, 'Z1T': Z1T, 'Z2T': Z2T, 'Z3T': Z3T
    }
...
sdp.save_monomial_index("1sDI_2outRandomness_steeringineq_monomials.txt") # 298+1 monomials


# Define the picos optimization problem	
monomial_dict = load_monomial_dict("1sDI_2outRandomness_steeringineq_monomials.txt")
# ~ print(monomial_dict)
...
for idx, score in enumerate(score_list):
    ...
    for i in range(2*M-1):
        P.set_objective("min", picos.trace((A1 @ A1) * (ComplexVars['Z0'] + ComplexVars['Z0'].H + (1-T[i]) * HermitianVars['Z0T_Z0']) + T[i] * HermitianVars['Z0_Z0T']) + \
                               picos.trace((A1 @ (I2-A1)) * (ComplexVars['Z1'] + ComplexVars['Z1'].H + (1-T[i]) * HermitianVars['Z1T_Z1']) + T[i] * HermitianVars['Z1_Z1T']) + \
                               picos.trace(((I2-A1) @ A1) * (ComplexVars['Z2'] + ComplexVars['Z2'].H + (1-T[i]) * HermitianVars['Z2T_Z2']) + T[i] * HermitianVars['Z2_Z2T']) + \
                               picos.trace(((I2-A1) @ (I2-A1)) * (ComplexVars['Z3'] + ComplexVars['Z3'].H + (1-T[i]) * HermitianVars['Z3T_Z3']) + T[i] * HermitianVars['Z3_Z3T']))
        P.solve(solver = "mosek")
        Entr += (1+float(P)) * W[i] / (T[i] * log(2))

    r = 0.00033
    rate = Entr - 3*log2(3)*r - h(r)
    results.append(rate)
    print(score, Entr, rate)
...
# ~ ax.set_ylabel(r'$H(\hat{A}_3|E)$', fontsize = 14)
# ~ ax.grid(True)
# ~ ax.minorticks_on()
